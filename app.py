...
def getINVName(topic):
    my_list = topic.split("/")
    return my_list[-3]

def Freq_callback(client, userdata, message):
    try:
        controllers[getINVName(message.topic)].setFreq(message.payload)
    except ValueError:
        pass


def Run_callback(client, userdata, message):
    if int(message.payload)==1:
        controllers[getINVName(message.topic)].runMotor()
    else:
        controllers[getINVName(message.topic)].stopMotor()
...
def buildTopics(root, name):
    ...
    Topics.update({"Status":base+'Data/Status'})
    Topics.update({"State":base+'Data/State'})
    Topics.update({"Run":base+'+/Control/Run'})
    Topics.update({"Freq":base+'+/Control/Freq'})
    return (Topics)
...
if __name__ == "__main__":
    ...
    Topics = buildTopics(device_root, "INVERTERS")
    client = mqtt.Client(client_id="DSS_INVERTER_MQTT_GATEWAY")
    ...
    for device in conf["Devices"]:
        ...
        DEVICESlaveAdderess=device["SlaveAddress"]
        DEVICEName=device["Name"]
        
        controllers.update({DEVICEName:Inverter.Inverter(DEVICEIP,DEVICEPort,DEVICESlaveAdderess)})
        controllers[DEVICEName].debug=True
    ...
    client.message_callback_add(Topics["Run"], Run_callback)
    client.message_callback_add(Topics["Freq"], Freq_callback)
    ...
    try:
        #controller=Inverter.Inverter(DEVICEIP,DEVICEPort,DEVICESlaveAdderess)
        
        print (datetime.utcnow().strftime('[%Y-%m-%d %H:%M:%S.%f')[:-3]+"]\tOpened Connection to Inverter via ethernet RS484 gateway: "+DEVICEIP+":",DEVICEPort)
    except:
        print (datetime.utcnow().strftime('[%Y-%m-%d %H:%M:%S.%f')[:-3]+"]\tError while connecting to Inverter : "+DEVICEIP+":",DEVICEPort)
        closeAll()
        raise
        quit()
    ...
    for name, controller in controllers.items():
        try:
            ver=controller.getFirmware()
            print (datetime.utcnow().strftime('[%Y-%m-%d %H:%M:%S.%f')[:-3]+"]\tController "+name+" Firmware:"+str(ver))
        except:
            print (datetime.utcnow().strftime('[%Y-%m-%d %H:%M:%S.%f')[:-3]+"]\tError while querring "+name+"'s firmware")
            closeAll()
            raise
            quit()
